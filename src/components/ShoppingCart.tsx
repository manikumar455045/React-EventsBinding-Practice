...
class ShoppingCart extends React.Component<Iprops, Istate> {
    ...
    incQty = (sno : string) => {
        let incProduct : IProduct[] = this.state.product.map((product) => {
            if(sno === product.sno){
                return {
                    ...product,
                      qty : product.qty + 1
                                ...
                                this.state.product.map((product) => {
                                   return (
                                       <tr key={product.sno}>
                                           <td>{product.sno}</td>
                                           <td>{product.name}</td>
                                           <td><img src={product.img}  width="50px" className="img-fluid" alt=""/></td>
                                           <td><i className="fa fa-minus-circle" onClick={this.decQty.bind(this, product.sno)}/>
                                           {product.qty}
                                           <i className="fa fa-plus-circle" onClick={this.incQty.bind(this, product.sno)}/></td>
